...
impl SumeragiHandle {
    ...
    pub fn start(
        ...
        let sumeragi = main_loop::Sumeragi {
            ...
            events_sender,
            commit_time: state.view().parameters().commit_time(),
            block_time: state.view().parameters().block_time(),
            max_txs_in_block: state.view().parameters().max_transactions_in_block.get() as usize,
            kura: Arc::clone(&kura),
